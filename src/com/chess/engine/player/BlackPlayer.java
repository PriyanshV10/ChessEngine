    ...
    protected Collection<Move> calculateKingCastles(final Collection<Move> playerLegals, final Collection<Move> opponentsLegals) {
        ...
        if(this.playerKing.isFirstMove() && !this.isInCheck()) {
            ...
            if(!this.board.getTile(1).isTileOccupied() && !this.board.getTile(2).isTileOccupied() && !this.board.getTile(3).isTileOccupied()) {
                ...
                if(rookTile.isTileOccupied() && rookTile.getPiece().isFirstMove()) {
                    if(Player.calculateAttacksOnTile(3, opponentsLegals).isEmpty() && Player.calculateAttacksOnTile(2, opponentsLegals).isEmpty() && rookTile.getPiece().getPieceType().isRook()) {
